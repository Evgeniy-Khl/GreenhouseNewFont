...
#define ID_SOIL3         0xF3   // идентификатор модул€ грунта #3
#define ID_SOIL4         0xF4   // идентификатор модул€ грунта #4
...
// –абота таймера прив€занна€ к xternal Interrupt 0
...
unsigned char tableRH(signed int maxT, signed int minT)
 {
  signed int dT;
   if (maxT>199 && maxT<410) // maxT> 19.9 и maxT< 41.0
    {
     dT = (maxT-minT)*16/10;
     if (dT<0) dT = 240;        // задаем число при котором dT >>=3; выполн€етс€ -> dT>20
     maxT /=10;
     dT >>=3;
     if (dT>20) dT = 255;
     else if (dT==0) dT = 100;
...
//    if(x) tt /= x; else {tt = 1900; errors |= 0x08;}  // 0x08 - ошибка датчиков среднего значени€ внутр. воздуха
//    return tt;
//}